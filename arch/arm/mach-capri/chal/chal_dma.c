...
typedef enum {
	...
	DMAC_INST_DMAWFE = 18,
	DMAC_INST_DMAWFP = 19,
	DMAC_INST_DMAWFPP = 20,
	DMAC_INST_DMAWMB = 21,
	DMAC_INST_DMALPFEEND = 22,
	DMAC_INST_MAX = 23
} DMAC_INST_e;
...
static const DMA_INSTR_ENCODE_t dmacInstEncode[DMAC_INST_MAX + 1] = {
	...
	DECLARE_INST(DMAWFP, 0x30, 2),	/* Handles burst or single transfer */
	DECLARE_INST(DMAWFPP, 0x31, 2),	/* Handles both burst and single xfer */
	DECLARE_INST(DMAWMB, 0x13, 1),
	DECLARE_INST(DMALPFEEND, 0x2C, 2)
};

/*******************************************************************************
 * Local Functions
 *******************************************************************************/
static int chal_dma_prepare_circular_transfer(CHAL_CHANNEL_HANDLE handle);

static int chal_dma_prepare_list_transfer(CHAL_CHANNEL_HANDLE handle);

static int chal_dma_prepare_peripheral_controlled_transfer(CHAL_CHANNEL_HANDLE handle);
 ...
 * ******************************************************************************
 */
static CHAL_DMA_STATUS_t chal_dma_get_loop_cnt(uint32_t cnt,
					       uint32_t *loop0,
					       uint32_t *loop1,
					       uint32_t *rem)
{
 ...
 * ******************************************************************************
 */
static void chal_dma_encode_instruction(uint8_t **ucode,
					DMAC_INST_e inst,
					uint32_t para0,
					uint32_t para1)
{
	...
	case DMAC_INST_DMAWFP:	/* para0 -- burst_len */
		/*para1 -- peri_num */
		(*ucode)[0] =
		    (dmacInstEncode[DMAC_INST_DMAWFP].
		     inst_enc & 0xff) | (para0 == CHAL_DMA_BURST_LEN_1 ? 0 : 2);
		(*ucode)[1] =
		    (dmacInstEncode[DMAC_INST_DMAWFP].
		     inst_enc >> 8) | (para1 << 3);
	...
	case DMAC_INST_DMAWFPP:
		/* para1 -- peri_num */
		(*ucode)[0] = dmacInstEncode[DMAC_INST_DMAWFPP].inst_enc;
		(*ucode)[1] =
		    (dmacInstEncode[DMAC_INST_DMAWFPP].
		     inst_enc >> 8) | (para1 << 3);
		(*ucode) += dmacInstEncode[DMAC_INST_DMAWFPP].inst_len;
		break;

	case DMAC_INST_DMALDP:	/* para0 -- burst_len */
		/* para1 -- peri_num */
		(*ucode)[0] =
		    (dmacInstEncode[DMAC_INST_DMALDP].
		     inst_enc & 0xff) | (para0 == CHAL_DMA_BURST_LEN_1 ? 0 : 2);
		(*ucode)[1] =
		    (dmacInstEncode[DMAC_INST_DMALDP].
		     inst_enc >> 8) | (para1 << 3);
	...
	case DMAC_INST_DMASTP:	/* para0 -- burst_len */
		/* para1 -- peri_num */
		(*ucode)[0] =
		    (dmacInstEncode[DMAC_INST_DMASTP].
		     inst_enc & 0xff) | (para0 == CHAL_DMA_BURST_LEN_1 ? 0 : 2);
		(*ucode)[1] =
		    (dmacInstEncode[DMAC_INST_DMASTP].
		     inst_enc >> 8) | (para1 << 3);
	...
	case DMAC_INST_DMALP:	/* para0 -- 0, 1, which cnt */
		/* para1 -- loops */
		(*ucode)[0] =
		    dmacInstEncode[DMAC_INST_DMALP].
		    inst_enc | (para0 ? 0x2 : 0);
		(*ucode)[1] = para1 - 1;
	...
	case DMAC_INST_DMALPEND:	/* para0 -- 0, 1, which cnt */
		/* para1 -- jump back */
		(*ucode)[0] =
		    dmacInstEncode[DMAC_INST_DMALPEND].
		    inst_enc | 0x10 | (para0 ? 0x04 : 0);
		(*ucode)[1] = para1;
 ...
 * ******************************************************************************
 */
CHAL_HANDLE chal_dma_init(CHAL_DMA_SECURE_STATE_t secureState)
{
	...
	if (secureState == CHAL_DMA_STATE_OPEN) {
		pDmaDev = &openDev;
		pDmaDev->baseAddr = (uint32_t)KONA_DMAC_NS_VA;
	} else {
		pDmaDev = &secureDev;
		pDmaDev->baseAddr = (uint32_t)KONA_DMAC_S_VA;
	}
	...
	if (!pDmaDev->config->initialized) {
		for (chan = 0; chan < CHAL_TOTAL_DMA_CHANNELS; chan++) {
			...
			pDmaDev->config->channelInfo[chan].
			    codeEndVirtualAddress = 0;
			pDmaDev->config->channelInfo[chan].maxDescCount = 0;
			pDmaDev->config->channelInfo[chan].curDescCount = 0;
		}

		/* populate capability info */
		pDmaDev->config->capabilities.numOfChannel =
		    CHAL_TOTAL_DMA_CHANNELS;
		pDmaDev->config->capabilities.maxBurstSize =
		    1 << CHAL_DMA_BURST_SIZE_8_BYTES;
		pDmaDev->config->capabilities.maxBurstLen =
		    CHAL_DMA_BURST_LEN_16 + 1;
		/* Get the handle to the DMUX block */
		pDmaDev->config->dmuxHandle =
		    chal_dmux_init((uint32_t)KONA_DMUX_VA);
	...
	/* To ensure DMUX semaphore is not in a dirty state */
	chal_dmux_unprotect(pDmaDev->config->dmuxHandle);

	/*
	 * clear any PENDING interrupt
	 */
	CHAL_REG_WRITE32(pDmaDev->baseAddr + NON_DMAC_INTCLR_OFFSET,
			 NON_DMAC_INTCLR_IRQ_CLR_MASK);
 ...
 * ******************************************************************************
 */
CHAL_CHANNEL_HANDLE chal_dma_config_channel(CHAL_HANDLE handle,
					    CHAL_DMA_CHANNEL_t channel,
					    CHAL_DMA_CHAN_CONFIG_t * config)
{
	...
	if (pDmaDev->secState == CHAL_DMA_STATE_OPEN) {
		pChanInfo->config.control =
		    CHAL_DMA_REG_CCR_SRC_OPEN | CHAL_DMA_REG_CCR_DST_OPEN;
	} else {
		pChanInfo->config.control = 0;
	}

	pChanInfo->config.control |=
	    (config->srcEndpoint & SRC_ADDR_MASK) << SRC_ADDR_MASK_SHIFT;
	pChanInfo->config.control |=
	    (config->srcBurstSize & SRC_BURST_SIZE_MASK) <<
	    SRC_BURST_SIZE_MASK_SHIFT;
	pChanInfo->config.control |=
	    (config->srcBurstLen & SRC_BURST_LEN_MASK) <<
	    SRC_BURST_LEN_MASK_SHIFT;
	pChanInfo->config.control |=
	    (config->dstEndpoint & DST_ADDR_MASK) << DST_ADDR_MASK_SHIFT;
	pChanInfo->config.control |=
	    (config->dstBurstSize & DST_BURST_SIZE_MASK) <<
	    DST_BURST_SIZE_MASK_SHIFT;
	pChanInfo->config.control |=
	    (config->dstBurstLen & DST_BURST_LEN_MASK) <<
	    DST_BURST_LEN_MASK_SHIFT;
	...
	pChanInfo->srcBurstDataLen =
	    (config->srcBurstLen + 1) * (1 << config->srcBurstSize);
	pChanInfo->dstBurstDataLen =
	    (config->dstBurstLen + 1) * (1 << config->dstBurstSize);
	pChanInfo->alwaysBurst = config->alwaysBurst;
	pChanInfo->interruptDisable = config->interruptDisable;
	pChanInfo->flushMode =
	    (config->flushMode & CHAL_DMA_FLUSH_NEVER) ? CHAL_DMA_FLUSH_NEVER :
	    config->flushMode;
 ...
 * ******************************************************************************
 */
void chal_dma_config_channel_memory(CHAL_CHANNEL_HANDLE handle,
...
*           
******************************************************************************/
void chal_dma_get_capabilities(CHAL_HANDLE handle,
...
*  @note
******************************************************************************/
uint32_t chal_dma_get_channel_descriptor_count(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
...
*  @note
******************************************************************************/
int chal_dma_get_current_channel_descriptor_index(CHAL_CHANNEL_HANDLE handle,
						  uint32_t *desc_idx)
{
	int rc = -1;
	int found = 0;
	...
	cur_src_addr =
	    CHAL_REG_READ32(pDmaDev->baseAddr + NON_DMAC_SA_0_OFFSET +
			    (channel << 5)) & NON_DMAC_SA_0_SRC_ADDR_MASK;
	cur_dst_addr =
	    CHAL_REG_READ32(pDmaDev->baseAddr + NON_DMAC_DA_0_OFFSET +
			    (channel << 5)) & NON_DMAC_DA_0_DST_ADDR_MASK;
	...
	for (descCount = 0; descCount < pChanInfo->curDescCount; descCount++) {
		...
		/*
		 * If both source and destination are incrementing addresses,
		...
		if (src_inc && dst_inc) {
			...
			if (src_delta >= 0 && src_delta <= buf_len
			    && dst_delta >= 0 && dst_delta <= buf_len) {
				found = 1;
				break;
			}
		}
 ...
 * ******************************************************************************
 */
void chal_dma_clear_int_status(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) pChanInfo->dmaHandle;
	CHAL_DMA_CHANNEL_t channel = pChanInfo->channel;
 ...
 * ******************************************************************************
 */
CHAL_DMA_STATUS_t chal_dma_get_channel(CHAL_HANDLE handle,
				       CHAL_DMA_CHANNEL_t * channel)
{
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) handle;

	CHAL_DMUX_STATUS_t ResultDemux =
	    chal_dmux_alloc_channel(pDmaDev->config->dmuxHandle,
				    (uint32_t *)channel);
 ...
 * ******************************************************************************
 */
CHAL_DMA_STATUS_t chal_dma_connect_peripheral(CHAL_CHANNEL_HANDLE handle,
					      CHAL_DMA_PERIPHERAL_t Peri)
{
	...
	ResultDemux =
	    chal_dmux_alloc_peripheral(pDmaDev->config->dmuxHandle, channel,
				       (CHAL_DMUX_END_POINT_t) Peri,
				       (CHAL_DMUX_END_POINT_t) PeriB, &src_id,
				       &dst_id);
 ...
 * ******************************************************************************
 */
void chal_dma_disconnect_peripheral(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) pChanInfo->dmaHandle;
	CHAL_DMA_CHANNEL_t channel = pChanInfo->channel;
 ...
 * ******************************************************************************
 */
CHAL_DMA_STATUS_t chal_dma_add_device_command_to_descriptor(CHAL_CHANNEL_HANDLE handle,
							    ...
							    uint32_t dstPhysicalAddr)
{
	...
	if ((pChanInfo->config.ChanCtrlBitField.srcAddrMode != 0)
	    && (pChanInfo->config.ChanCtrlBitField.dstAddrMode != 0)) {
		return CHAL_DMA_STATUS_FAILURE;
	}
	 ...
	 */
	chanDesc[index].control.control =
	    (chanDesc[index].
	     control.control & ~(SRC_ADDR_MASK << SRC_ADDR_MASK_SHIFT))
	    | (CHAL_DMA_ENDPOINT_MEMORY << SRC_ADDR_MASK_SHIFT);
	chanDesc[index].control.control =
	    (chanDesc[index].
	     control.control & ~(DST_ADDR_MASK << DST_ADDR_MASK_SHIFT))
	    | (CHAL_DMA_ENDPOINT_MEMORY << DST_ADDR_MASK_SHIFT);
	/* Re-configure burst length to CHAL_DMA_BURST_LEN_1 for src and dst */
	chanDesc[index].control.control =
	    (chanDesc[index].
	     control.control & ~(SRC_BURST_LEN_MASK <<
				 SRC_BURST_LEN_MASK_SHIFT))
	    | (CHAL_DMA_BURST_LEN_1 << SRC_BURST_LEN_MASK_SHIFT);
	chanDesc[index].control.control =
	    (chanDesc[index].
	     control.control & ~(DST_BURST_LEN_MASK <<
				 DST_BURST_LEN_MASK_SHIFT))
	    | (CHAL_DMA_BURST_LEN_1 << DST_BURST_LEN_MASK_SHIFT);

	/*
	 * Re-configure burst size to
	 * CHAL_DMA_BURST_SIZE_4_BYTES for src and dst
	 */
	chanDesc[index].control.control =
	    (chanDesc[index].
	     control.control & ~(SRC_BURST_SIZE_MASK <<
				 SRC_BURST_SIZE_MASK_SHIFT))
	    | (CHAL_DMA_BURST_SIZE_4_BYTES << SRC_BURST_SIZE_MASK_SHIFT);
	chanDesc[index].control.control =
	    (chanDesc[index].
	     control.control & ~(DST_BURST_SIZE_MASK <<
				 DST_BURST_SIZE_MASK_SHIFT))
 ...
 * ******************************************************************************
 */
CHAL_DMA_STATUS_t chal_dma_add_descriptor(CHAL_CHANNEL_HANDLE handle,
					  _Bool firstDescriptor,
					  uint32_t srcPhysicalAddr,
					  uint32_t dstPhysicalAddr,
					  uint32_t size)
{
	...
	if (size) {
		...
		chanDesc[index].control.control = pChanInfo->config.control;
		/*
		 * Do single  or modified burst transfer
		 * if data is not multiple of burst length
		 */
		if (size % chal_dma_get_burst_length(handle)) {
			uint32_t mask;
			uint32_t burstLen;
			mask =
			    ~((0xFFFFFFFF >> chanDesc[index].
			       control.ChanCtrlBitField.srcBurstSize) <<
			      chanDesc[index].control.ChanCtrlBitField.
			      srcBurstSize);
			...
			if (pChanInfo->alwaysBurst) {
				burstLen =
				    (size /
				     (1 << chanDesc[index].control.
				      ChanCtrlBitField.srcBurstSize)) - 1;
				/* Re-configure burst length to new burstLen */
				chanDesc[index].control.control =
				    (chanDesc[index].control.
				     control & ~(SRC_BURST_LEN_MASK <<
						 SRC_BURST_LEN_MASK_SHIFT))
				    | (burstLen << SRC_BURST_LEN_MASK_SHIFT);
				chanDesc[index].control.control =
				    (chanDesc[index].control.
				     control & ~(DST_BURST_LEN_MASK <<
						 DST_BURST_LEN_MASK_SHIFT))
 ...
 * ******************************************************************************
 */
CHAL_DMA_STATUS_t chal_dma_transfer_complete(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) pChanInfo->dmaHandle;
	CHAL_DMA_CHANNEL_t channel = pChanInfo->channel;

	return CHAL_REG_READ32(pDmaDev->baseAddr + NON_DMAC_CS0_OFFSET +
			       (channel << 3)) &
	    NON_DMAC_CS0_CHANNEL_STATUS_MASK;
}
 ...
 * ******************************************************************************
 */
uint32_t chal_dma_get_burst_length(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;

	return (pChanInfo->srcBurstDataLen >
		pChanInfo->dstBurstDataLen) ? pChanInfo->
	    srcBurstDataLen : pChanInfo->dstBurstDataLen;
}
 ...
 * ******************************************************************************
 */
uint32_t chal_dma_calculate_max_data_per_descriptor(CHAL_CHANNEL_HANDLE handle)
{
	return chal_dma_get_burst_length(handle) * CHAL_DMA_REG_MAX_LOOP_COUNT *
	    CHAL_DMA_REG_MAX_LOOP_COUNT;
}
 ...
 * ******************************************************************************
 */
uint32_t chal_dma_calculate_channel_memory(CHAL_CHANNEL_HANDLE handle,
					   uint32_t descCount)
{
	uint32_t length = 0;
	uint32_t src_inc = 0;
	...
	if ((pChanInfo->descType == CHAL_DMA_DESC_RING)
	    || (pChanInfo->descType == CHAL_DMA_DESC_PCTL)) {
		if ((src_inc) && (dst_inc)) {
			/* memory to memory */
			length +=
			    descCount * (3 *
					 dmacInstEncode[DMAC_INST_DMAMOV].
					 inst_len +
					 2 * ld_cnt *
					 dmacInstEncode[DMAC_INST_DMALD].
					 inst_len +
					 2 * ld_cnt *
					 dmacInstEncode[DMAC_INST_DMARMB].
					 inst_len +
					 2 * st_cnt *
					 dmacInstEncode[DMAC_INST_DMAST].
					 inst_len +
					 2 * st_cnt *
					 dmacInstEncode[DMAC_INST_DMAWMB].
					 inst_len +
					 3 *
					 (dmacInstEncode[DMAC_INST_DMALP].
					  inst_len +
					 ...
					 inst_len +
					 (is_send ?
					  dmacInstEncode[DMAC_INST_DMASEV].
					  inst_len : 0)
					 +
					 dmacInstEncode[DMAC_INST_DMASEV].
					 inst_len)
			...
			length +=
			    ...
			    (dmacInstEncode[DMAC_INST_DMAFLUSHP].inst_len +
			     ...
			     2 * ld_cnt *
			     dmacInstEncode[DMAC_INST_DMALD].inst_len +
			     2 * st_cnt *
			     (dmacInstEncode[DMAC_INST_DMAWFP].inst_len +
			      dmacInstEncode[DMAC_INST_DMASTP].inst_len)
			     + 3 * (dmacInstEncode[DMAC_INST_DMALP].inst_len +
				    dmacInstEncode[DMAC_INST_DMALPEND].inst_len)
			     + dmacInstEncode[DMAC_INST_DMAWMB].inst_len +
			     (is_send ?
			      dmacInstEncode[DMAC_INST_DMASEV].inst_len : 0)
			...
			length +=
			    ...
			    (dmacInstEncode[DMAC_INST_DMAFLUSHP].inst_len +
			     ...
			     2 * ld_cnt *
			     (dmacInstEncode[DMAC_INST_DMAWFP].inst_len +
			      dmacInstEncode[DMAC_INST_DMALDP].inst_len)
			     +
			     2 * st_cnt *
			     dmacInstEncode[DMAC_INST_DMAST].inst_len +
			     3 * (dmacInstEncode[DMAC_INST_DMALP].inst_len +
				  dmacInstEncode[DMAC_INST_DMALPEND].inst_len)
			     + dmacInstEncode[DMAC_INST_DMAWMB].inst_len +
			     (is_send ?
		...
		if ((src_inc) && (dst_inc)) {
			...
			length +=
			    ...
			    + dmacInstEncode[DMAC_INST_DMAWMB].inst_len
			    +
			    (is_send ? dmacInstEncode[DMAC_INST_DMASEV].inst_len
			     : 0)
			    + dmacInstEncode[DMAC_INST_DMASEV].inst_len +
...
static int chal_dma_prepare_list_transfer(CHAL_CHANNEL_HANDLE handle)
{
	...
	inst =
	    (uint8_t *)(pChanInfo->chanVirtualAddr +
			pChanInfo->maxDescCount * sizeof(DMA_DESC_t));
	...
	/* Start encoding */
	is_wait ? chal_dma_encode_instruction(&inst, DMAC_INST_DMAWFE,
					      pChanInfo->event_wait, 0) : 0;
	...
	if (pChanInfo->flushMode & CHAL_DMA_FLUSH_FIRST) {
		...
		if ((pChanInfo->config.ChanCtrlBitField.srcAddrMode == 0)
		    || (pChanInfo->config.ChanCtrlBitField.dstAddrMode == 0)) {
			/* DMAFLUSHP */
			chal_dma_encode_instruction(&inst, DMAC_INST_DMAFLUSHP,
						    peripheralId, 0);
		}
	}
	...
	for (descCount = 0; descCount < pChanInfo->curDescCount; descCount++) {
		...
		/* Address mode */
		src_inc =
		    chanDesc[descCount].control.ChanCtrlBitField.srcAddrMode;
		dst_inc =
		    chanDesc[descCount].control.ChanCtrlBitField.dstAddrMode;

		/* Obtain channel properties */
		src_burst_len =
		    chanDesc[descCount].control.ChanCtrlBitField.srcBurstLen;
		src_burst_size =
		    chanDesc[descCount].control.ChanCtrlBitField.srcBurstSize;
		dst_burst_len =
		    chanDesc[descCount].control.ChanCtrlBitField.dstBurstLen;
		dst_burst_size =
		    chanDesc[descCount].control.ChanCtrlBitField.dstBurstSize;
		...
		if (chal_dma_get_loop_cnt(cnt, &loop0, &loop1, &rem)) {
			debug_print(("%s: invalid loop cnt\n", __func__));
			return CHAL_DMA_STATUS_INVALID_PARAMETER;
		}
		...
		if ((src_inc == 0) || (dst_inc == 0)) {
			...
			if (pChanInfo->flushMode == CHAL_DMA_FLUSH_ALWAYS) {
				/* DMAFLUSHP */
				chal_dma_encode_instruction(&inst,
							    DMAC_INST_DMAFLUSHP,
							    peripheralId, 0);
			}
		}
		/* mov CCR */
		chal_dma_encode_instruction(&inst, DMAC_INST_DMAMOV, 1,
					    control);
		/* mov SRC */
		chal_dma_encode_instruction(&inst, DMAC_INST_DMAMOV, 0,
					    src_addr);
		/* mov DST */
		chal_dma_encode_instruction(&inst, DMAC_INST_DMAMOV, 2,
					    dst_addr);
		/* loop 1 */
		...
		if ((pChanInfo->flushMode & CHAL_DMA_FLUSH_CMD)
		    && (chanDesc[descCount].type == DMAC_DESC_TYPE_CMD)) {
			/* DMAWFP (Wait for a burst request signal ) */
			chal_dma_encode_instruction(&inst, DMAC_INST_DMAWFP,
						    CHAL_DMA_BURST_LEN_2,
						    peripheralId);
			/* DMAST */
			chal_dma_encode_instruction(&inst, DMAC_INST_DMAST, 0,
						    0);
			/* Wait until write completed */
			chal_dma_encode_instruction(&inst, DMAC_INST_DMAWMB, 0,
						    0);
			/* DMAFLUSHP */
			chal_dma_encode_instruction(&inst, DMAC_INST_DMAFLUSHP,
						    peripheralId, 0);
		...
		/* loopend 0 */
		chal_dma_encode_instruction(&inst, DMAC_INST_DMALPEND, 0,
					    inst - inst_loop0);
		/* loopend 1 */
		chal_dma_encode_instruction(&inst, DMAC_INST_DMALPEND, 1,
					    inst - inst_loop1);
		/* Encode remaining loop */
		if (rem) {
			/* rem */
			chal_dma_encode_instruction(&inst, DMAC_INST_DMALP, 0,
						    rem);
			...
			chal_dma_encode_instruction(&inst, DMAC_INST_DMALPEND,
						    0, inst - inst_loop0);
		}
		/* DMA WMB */
		chal_dma_encode_instruction(&inst, DMAC_INST_DMAWMB,
					    pChanInfo->event_send, 0);
	}
	/* DMA SEV */
	is_send ? chal_dma_encode_instruction(&inst, DMAC_INST_DMASEV,
					      pChanInfo->event_send, 0) : 0;
	/* DMA SEV (interrupt) */
	if (!pChanInfo->interruptDisable) {
		chal_dma_encode_instruction(&inst, DMAC_INST_DMASEV, channel,
					    0);
	}
...
static int chal_dma_prepare_circular_transfer(CHAL_CHANNEL_HANDLE handle)
{
	...
	if ((src_bytes % dst_bytes) && (dst_bytes % src_bytes)) {
		debug_print(("%s: invalid parameter\n", __func__));
		return CHAL_DMA_STATUS_INVALID_PARAMETER;
	}
	...
	for (descCount = 0; descCount < pChanInfo->curDescCount; descCount++) {
		...
		for (j = 0; j < ld_cnt; j++) {
			if (src_inc) {
				...
				chal_dma_encode_instruction(&inst,
							    DMAC_INST_DMAWFP,
							    pChanInfo->
							    config.ChanCtrlBitField.srcBurstLen,
							    peripheralId);
				/* DMALDP */
				chal_dma_encode_instruction(&inst,
							    DMAC_INST_DMALDP,
		...
		for (j = 0; j < st_cnt; j++) {
			if (dst_inc) {
				...
				chal_dma_encode_instruction(&inst,
							    DMAC_INST_DMAWFP,
							    pChanInfo->
							    config.ChanCtrlBitField.dstBurstLen,
							    peripheralId);
				/* DMASTP */
				chal_dma_encode_instruction(&inst,
							    DMAC_INST_DMASTP,
		...
		if (rem) {
			...
			for (j = 0; j < ld_cnt; j++) {
				if (src_inc) {
					...
					chal_dma_encode_instruction(&inst,
								    DMAC_INST_DMAWFP,
								    pChanInfo->config.ChanCtrlBitField.srcBurstLen,
								    peripheralId);
					/* DMALDP */
					chal_dma_encode_instruction(&inst,
								    DMAC_INST_DMALDP,
			...
			for (j = 0; j < st_cnt; j++) {
				if (dst_inc) {
					...
					chal_dma_encode_instruction(&inst,
								    DMAC_INST_DMAWFP,
								    pChanInfo->config.ChanCtrlBitField.dstBurstLen,
								    peripheralId);
					/* DMASTP */
					chal_dma_encode_instruction(&inst,
								    DMAC_INST_DMASTP,
		...
		/* DMA SEV */
		is_send ? chal_dma_encode_instruction(&inst, DMAC_INST_DMASEV,
						      pChanInfo->event_send,
						      0) : 0;
		/* DMA SEV ( interrupt ) */
		if (!pChanInfo->interruptDisable) {
			chal_dma_encode_instruction(&inst, DMAC_INST_DMASEV,
						    channel, 0);
		}
	}
	/* DMALPFEEND */
	/* Continue from the begining */
	chal_dma_encode_instruction(&inst, DMAC_INST_DMALPFEEND, 0,
				    inst - inst_lpfe);
	/* Mark the last micro code address */
	pChanInfo->codeEndVirtualAddress = (uint32_t)inst;
 ...
 * ******************************************************************************
 * 
 *  Function Name:  chal_dma_prepare_peripheral_controlled_transfer
 *
 *  Description:  Prepare instruction encoding for peripheral controlled transfer
 *
 * ******************************************************************************
 */
static int chal_dma_prepare_peripheral_controlled_transfer(CHAL_CHANNEL_HANDLE handle)
{
	uint32_t src_inc;
	uint32_t dst_inc;
	uint32_t j, src_bytes, dst_bytes;
	uint32_t ld_cnt, st_cnt;
	uint8_t *inst = 0;
	uint8_t is_wait, *inst_lpfe;
	DMA_DESC_t *chanDesc;
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
	CHAL_DMA_CHANNEL_t channel = pChanInfo->channel;
	uint32_t peripheralId = 2 * channel;

	if (pChanInfo->curDescCount == 1) {
		/* Locate instruction (ucode) memory */
		inst =
		    (uint8_t *)(pChanInfo->chanVirtualAddr +
				pChanInfo->maxDescCount * sizeof(DMA_DESC_t));
		...
		/* wait for event */
		is_wait ? chal_dma_encode_instruction(&inst, DMAC_INST_DMAWFE,
						      pChanInfo->event_wait,
						      0) : 0;
		/* Flush peripheral at the begining of the transfer */
		if (pChanInfo->flushMode & CHAL_DMA_FLUSH_FIRST) {
			/* Channel must be connected to a device */
			if ((pChanInfo->config.ChanCtrlBitField.srcAddrMode ==
			     0)
			    || (pChanInfo->config.
				ChanCtrlBitField.dstAddrMode == 0)) {
				/* DMAFLUSHP */
				chal_dma_encode_instruction(&inst,
							    DMAC_INST_DMAFLUSHP,
							    peripheralId, 0);
		...
		/* Address mode */
		src_inc = chanDesc[0].control.ChanCtrlBitField.srcAddrMode;
		dst_inc = chanDesc[0].control.ChanCtrlBitField.dstAddrMode;
		...
		for (j = 0; j < ld_cnt; j++) {
			if (src_inc) {
				...
				/* DMAWFPP (wait for single or burst request) */
				chal_dma_encode_instruction(&inst,
							    DMAC_INST_DMAWFPP,
							    pChanInfo->
							    config.ChanCtrlBitField.srcBurstLen,
		...
		return (src_bytes < dst_bytes) ? dst_bytes : src_bytes;
	}
	return 0;
}
 ...
 * ******************************************************************************
 */
int chal_dma_prepare_transfer(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) pChanInfo->dmaHandle;
	int rc = 0;
	...
	else if (pChanInfo->descType == CHAL_DMA_DESC_PCTL)
		rc = chal_dma_prepare_peripheral_controlled_transfer(handle);
	else
		rc = chal_dma_prepare_list_transfer(handle);

	if (rc > 0) {
		/* Wait until DMA is ready to accept command */
		while (CHAL_REG_READ32
		       (pDmaDev->baseAddr + NON_DMAC_DBGSTATUS_OFFSET) & 0x1) ;
	}
 ...
 * ******************************************************************************
 */
CHAL_DMA_STATUS_t chal_dma_start_transfer(CHAL_CHANNEL_HANDLE handle)
{
	...
	rc = chal_dma_get_channel_status(handle);
	if (rc != CHAL_DMA_STATUS_SUCCESS)
		return rc;
	...
	if (chal_dmux_protect(pDmaDev->config->dmuxHandle) ==
		...
		if (pDmaDev->secState == CHAL_DMA_STATE_SECURE) {
			val =
			    dmacInstEncode[DMAC_INST_DMAGO].inst_enc |
			    CHAL_DMA_DMAGO_SECURE;
		} else {
			val =
			    dmacInstEncode[DMAC_INST_DMAGO].inst_enc |
			    CHAL_DMA_DMAGO_OPEN;
		...
		CHAL_REG_WRITE32(pDmaDev->baseAddr + NON_DMAC_DBGINST0_OFFSET,
				 val);

		/* Locate physical address of the micro-code */
		val =
		    pChanInfo->chanPhysicalAddr +
		    pChanInfo->maxDescCount * sizeof(DMA_DESC_t);
		/* Specify the location of the ucode */
		CHAL_REG_WRITE32(pDmaDev->baseAddr + NON_DMAC_DBGINST1_OFFSET,
				 val);

		/* Configure interrupt */
		if (pChanInfo->interruptDisable) {
			val =
			    CHAL_REG_READ32(pDmaDev->baseAddr +
					    NON_DMAC_INTEN_OFFSET) &
			    (~(1 << channel));
		} else {
			val =
			    CHAL_REG_READ32(pDmaDev->baseAddr +
		...
		CHAL_REG_WRITE32(pDmaDev->baseAddr + NON_DMAC_INTEN_OFFSET,
				 val);
		/* Execute instructions written in Debug register 0 and 1 */
		val = 0;
		CHAL_REG_WRITE32(pDmaDev->baseAddr + NON_DMAC_DBGCMD_OFFSET,
				 val);
 ...
 * ******************************************************************************
 */
CHAL_CHANNEL_HANDLE chal_dma_channel_to_handle(CHAL_HANDLE handle,
					       CHAL_DMA_CHANNEL_t channel)
{
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) handle;
	DMA_CHAN_INFO_t *pChanInfo = &pDmaDev->config->channelInfo[channel];
...
CHAL_DMA_STATUS_t chal_dma_get_channel_status(CHAL_CHANNEL_HANDLE handle)
{
	...
	status =
	    CHAL_REG_READ32(pDmaDev->baseAddr + NON_DMAC_CS0_OFFSET +
			    (channel << 3));
 ...
 * ******************************************************************************
 */
void chal_dma_interrupt_enable(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) pChanInfo->dmaHandle;
	CHAL_DMA_CHANNEL_t channel = pChanInfo->channel;
 ...
 * ******************************************************************************
 */
void chal_dma_interrupt_disable(CHAL_CHANNEL_HANDLE handle)
{
	DMA_CHAN_INFO_t *pChanInfo = (DMA_CHAN_INFO_t *) handle;
	DMA_DEV_t *pDmaDev = (DMA_DEV_t *) pChanInfo->dmaHandle;
	CHAL_DMA_CHANNEL_t channel = pChanInfo->channel;
 ...
 * ******************************************************************************
 */
void chal_dma_dump_register(CHAL_CHANNEL_HANDLE handle,
			    int (*fpPrint) (const char *, ...))
{
	...
	startCode =
	    (uint8_t *)(pChanInfo->chanVirtualAddr +
			pChanInfo->maxDescCount * sizeof(DMA_DESC_t));
	phyAddr =
	    (uint8_t *)(pChanInfo->chanPhysicalAddr +
			pChanInfo->maxDescCount * sizeof(DMA_DESC_t));
