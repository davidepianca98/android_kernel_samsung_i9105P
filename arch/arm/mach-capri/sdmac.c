...
SDMA_DeviceAttribute_t SDMA_gDeviceAttribute[DMA_NUM_DEVICE_ENTRIES] = {
	...
	[DMA_DEVICE_SPUM_MEM_TO_DEV] = {
					...
					.peripheralId =
					CHAL_DMA_PERIPHERAL_SPUM_OpenW,
					},
	[DMA_DEVICE_SPUM_DEV_TO_MEM] = {
					...
					.peripheralId =
					CHAL_DMA_PERIPHERAL_SPUM_OpenR,
					},
	...
	[DMA_DEVICE_SSP5A_RX0] =
	    DEVICE_SSPI_DEV_TO_MEM("ssp5a_rx0", CHAL_DMA_PERIPHERAL_SSP_5A_RX0),
	[DMA_DEVICE_SSP5B_TX0] =
	    DEVICE_SSPI_MEM_TO_DEV("ssp5b_tx0", CHAL_DMA_PERIPHERAL_SSP_5B_TX0),
	[DMA_DEVICE_SSP5C_RX1] =
	    DEVICE_SSPI_DEV_TO_MEM("ssp5c_rx0", CHAL_DMA_PERIPHERAL_SSP_5C_RX1),
	[DMA_DEVICE_SSP5D_TX1] =
	    DEVICE_SSPI_MEM_TO_DEV("ssp5d_tx1", CHAL_DMA_PERIPHERAL_SSP_5D_TX1),

	[DMA_DEVICE_SSP6A_RX0] =
	    DEVICE_SSPI_DEV_TO_MEM("ssp6a_rx0", CHAL_DMA_PERIPHERAL_SSP_6A_RX0),
	[DMA_DEVICE_SSP6B_TX0] =
	    DEVICE_SSPI_MEM_TO_DEV("ssp6b_tx0", CHAL_DMA_PERIPHERAL_SSP_6B_TX0),
	[DMA_DEVICE_SSP6C_RX1] =
	    DEVICE_SSPI_DEV_TO_MEM("ssp6c_rx0", CHAL_DMA_PERIPHERAL_SSP_6C_RX1),
	[DMA_DEVICE_SSP6D_TX1] =
	    DEVICE_SSPI_MEM_TO_DEV("ssp6d_tx1", CHAL_DMA_PERIPHERAL_SSP_6D_TX1),
    ...
    )
{
	...
	if (numBytes % chal_dma_get_burst_length(channel->sdmacHandle)) {
		...
		numDescriptors =
		    2 +
		    (numBytes /
		     chal_dma_calculate_max_data_per_descriptor
		     (channel->sdmacHandle));
	} else {
		numDescriptors =
		    1 +
...
int sdma_init(void)
{
	...
	rc = request_irq(INTV_DMAC_OPEN, sdma_interrupt_handler,
			 IRQF_DISABLED, "Open DMA Handler", &gOpenDmaHandle);
	if (rc != 0)
	...
	rc = request_irq(INTV_DMAC_SECURE, sdma_interrupt_handler,
			 IRQF_DISABLED, "Secure DMA Handler", &gSecDmaHandle);
	if (rc != 0)
...
*/
/****************************************************************************/
int sdma_map_create_descriptor_ring_with_command(SDMA_Handle_t handle, DMA_MMAP_CFG_T * memMap,	/* Memory map that will be used */
