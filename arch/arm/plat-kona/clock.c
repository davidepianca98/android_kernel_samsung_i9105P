...
int clk_lock(struct clk *clk, unsigned long *flags)
{
...
int clk_unlock(struct clk *clk, unsigned long *flags)
{
	struct ccu_clk *ccu_clk = get_ccu_clk(clk);
	if (ccu_clk)
...
int clk_register(struct clk_lookup *clk_lkup, int num_clks)
{
	...
	for (i = 0; i < num_clks; i++) {
		...
		if (clk_lkup[i].clk->clk_type == CLK_TYPE_CCU) {
			...
			spin_lock_init(&ccu_clk->wr_lock);
		}
...
int ccu_set_freq_policy(struct ccu_clk *ccu_clk, int policy_id, int freq_id, int opp_inx)
{
	int ret;
	if (IS_ERR_OR_NULL(ccu_clk) || !ccu_clk->ccu_ops || !ccu_clk->ccu_ops->set_freq_policy)
		return -EINVAL;
	CCU_ACCESS_EN(ccu_clk, 1);
	ret = ccu_clk->ccu_ops->set_freq_policy(ccu_clk, policy_id, freq_id, opp_inx);
	CCU_ACCESS_EN(ccu_clk, 0);
	return ret;
...
int ccu_get_voltage(struct ccu_clk *ccu_clk, int freq_id)
{
	...
	if (IS_ERR_OR_NULL(ccu_clk)
		|| !ccu_clk->ccu_ops || !ccu_clk->ccu_ops->get_voltage)
		return -EINVAL;
	CCU_ACCESS_EN(ccu_clk, 1);
...
static int ccu_clk_set_freq_policy(struct ccu_clk *ccu_clk, int policy_id, int freq_id, int opp_inx)
{
	u32 reg_val = 0;
	u32 shift;
...
static int ccu_clk_set_voltage(struct ccu_clk *ccu_clk, int volt_id, u8 voltage)
{
	...
	case CCU_VLT0:
		...
		reg_addr = (u32)CCU_VLT0_3_REG(ccu_clk);
		break;
	case CCU_VLT1:
	...
	case CCU_VLT4:
		...
		reg_addr = (u32)CCU_VLT4_7_REG(ccu_clk);
		break;
	case CCU_VLT5:
...
static int ccu_clk_init(struct clk *clk)
{
	...
	if (ccu_clk->policy_freq_offset != 0) {
		/*Init freq policy */
		if (ccu_clk->clk.id == CLK_KPROC_CCU_CLK_ID)
			/*proc CCU booting policy is controlled by loader*/
			for (inx = 0; inx < MAX_CCU_POLICY_COUNT-1; inx++) {
				BUG_ON(ccu_clk->freq_policy[inx] >=
					   ccu_clk->freq_count);
				ccu_set_freq_policy(ccu_clk, inx,
					ccu_clk->freq_policy[inx],
					pi_get_active_opp(ccu_clk->pi_id));
			}
		else
...
static int peri_clk_enable(struct clk *clk, int enable)
{
	...
	struct peri_clk *peri_clk;
	int insurance;
	int insurance_threashold=2000;
	int insurance_max_threshold=10000;
...
static int core_clk_set_rate(struct clk *clk, u32 rate)
{
	...
	u32 div;
	struct core_clk *core_clk;
	static u32 l_p_j_ref;
	static u32 l_p_j_ref_freq;
	int ret;
...
int __clk_enable_autogate(struct clk *clk)
{
	struct ref_clk *ref_clk;
	struct peri_clk *peri_clk;
	struct bus_clk *bus_clk;
	...
	case CLK_TYPE_PERI:
		peri_clk = to_peri_clk(clk);
		ccu_write_access_enable(peri_clk->ccu_clk, true);
		ret = peri_clk_set_gating_ctrl(peri_clk, CLK_GATING_AUTO);
		ccu_write_access_enable(peri_clk->ccu_clk, false);
		break;
	...
	case CLK_TYPE_BUS:
		bus_clk = to_bus_clk(clk);
		ccu_write_access_enable(bus_clk->ccu_clk, true);
		ret = bus_clk_set_gating_ctrl(bus_clk, CLK_GATING_AUTO);
		ccu_write_access_enable(bus_clk->ccu_clk, false);
		break;
	...
	case CLK_TYPE_REF:
		ref_clk = to_ref_clk(clk);
		ccu_write_access_enable(ref_clk->ccu_clk, true);
		ret = ref_clk_set_gating_ctrl(ref_clk, CLK_GATING_AUTO);
		ccu_write_access_enable(ref_clk->ccu_clk, false);
		break;
...
int clk_enable_autogate(struct clk *clk)
{
	int ret;
	unsigned long flags;
	...
	clk_lock(clk, &flags);
	ret = __clk_enable_autogate(clk);
	clk_unlock(clk, &flags);

	return ret;
}
EXPORT_SYMBOL(clk_enable_autogate);

int __clk_disable_autogate(struct clk *clk)
{
	struct ref_clk *ref_clk;
	struct peri_clk *peri_clk;
	struct bus_clk *bus_clk;
	...
	case CLK_TYPE_PERI:
		peri_clk = to_peri_clk(clk);
		ccu_write_access_enable(peri_clk->ccu_clk, true);
		ret = peri_clk_set_gating_ctrl(peri_clk, CLK_GATING_SW);
		ccu_write_access_enable(peri_clk->ccu_clk, false);
		break;
	...
	case CLK_TYPE_BUS:
		bus_clk = to_bus_clk(clk);
		ccu_write_access_enable(bus_clk->ccu_clk, true);
		ret = bus_clk_set_gating_ctrl(bus_clk, CLK_GATING_SW);
		ccu_write_access_enable(bus_clk->ccu_clk, false);
		break;
	...
	case CLK_TYPE_REF:
		ref_clk = to_ref_clk(clk);
		ccu_write_access_enable(ref_clk->ccu_clk, true);
		ret = ref_clk_set_gating_ctrl(ref_clk, CLK_GATING_SW);
		ccu_write_access_enable(ref_clk->ccu_clk, false);
		break;
...
int clk_disable_autogate(struct clk *clk)
{
	int ret;
	unsigned long flags;
	...
	clk_lock(clk, &flags);
	ret = __clk_disable_autogate(clk);
	clk_unlock(clk, &flags);

	return ret;
}
EXPORT_SYMBOL(clk_disable_autogate);
...
static const struct file_operations clock_mon_debug_fops = {
	.open = clk_debugfs_open,
...
static int clk_debug_set_autogate(void *data, u64 val)
{
	struct clk *clock = data;
	if (val == 1)
		clk_enable_autogate(clock);
	else if (val == 0)
		clk_disable_autogate(clock);
	else
		clk_dbg("Invalid value\n");
...
DEFINE_SIMPLE_ATTRIBUTE(clock_enable_autogate_fops,
		NULL, clk_debug_set_autogate, "%llu\n");
...
int __init clock_debug_add_ccu(struct clk *c)
{
	...
	*dent_volt_tbl = 0;
	...
	dent_volt_tbl = debugfs_create_file("volt_id_update",
		(S_IWUSR | S_IRUSR), ccu_clk->dent_ccu_dir,
				c, &ccu_volt_tbl_update_fops);
	if (!dent_volt_tbl)
		goto err;
...
int __init clock_debug_add_clock(struct clk *c)
{
	...
	*dent_status = 0, *dent_flags = 0, *dent_use_cnt = 0, *dent_id = 0,
    ...
    /* file /clock/clk_a/autogate */
	dent_enable = debugfs_create_file("autogate", S_IRUGO | S_IWUSR,
				  dent_clk_dir, c, &clock_enable_autogate_fops);
	if (!dent_enable)
		goto err;
