...
static int amxr_alloc_resampler(struct amxr_cnxlist_node *cnxlp,
				AMXR_RESAMP_TYPE rtype);
static void amxr_free_resamp(struct amxr_cnxlist_node *cnxlp);
static int amxr_add_cnx(struct amxr_port_node *srcportp,
			struct amxr_port_node *dstportp, AMXR_CONNECT_DESC desc,
			AMXR_RESAMP_TYPE rtype);
static int amxr_add_cnx_unsafe(struct amxr_port_node *srcportp,
			       struct amxr_port_node *dstportp,
			       AMXR_CONNECT_DESC desc, AMXR_RESAMP_TYPE rtype,
			       unsigned int lossdb);
static int amxr_del_cnxl_unsafe(struct amxr_cnxlist_node *cnxlp, int free);
static int amxr_del_dst_from_cnxl_unsafe(struct amxr_cnxlist_node *cnxlp,
					 struct amxr_port_node *dstportp);
...
static int amxr_save_disabled_cnx_unsafe(struct amxr_port_node *srcportp,
					 struct amxr_cnxdstl_t *dstl_itemp,
					 AMXR_CONNECT_DESC desc);
static void amxr_del_dstport_unsafe(struct amxr_cnxlist_node *cnxlp, int i,
				    LLIST_NODE *listp, int *count, int free);
static void amxr_info_from_cnxl(AMXR_PORT_CNXINFO ** cnxinfopp,
				struct amxr_cnxlist_node *cnxlp,
				struct amxr_port_node *dstportp, int active);
static int amxr_set_cnxloss(struct amxr_cnxlist_node *cnxlp,
			    struct amxr_port_node *srcportp,
			    struct amxr_port_node *dstportp,
			    AMXR_CONNECT_DESC desc, unsigned int db);
static int amxr_get_cnxloss(struct amxr_cnxlist_node *cnxlp,
			    struct amxr_port_node *srcportp,
			    struct amxr_port_node *dstportp,
			    AMXR_CONNECT_DESC desc, unsigned int *db);
...
*/
static inline struct amxr_port_node *getPort(AMXR_PORT_ID id	/*<< (i) Port ID */
    )
{
...
*/
static inline struct amxr_port_node *getPort_nocheck(AMXR_PORT_ID id	/*<< (i) Port ID */
    )
{
	return id;
}
...
*/
static inline AMXR_PORT_ID getId(struct amxr_port_node *portp	/*<< (i) Port pointer */
    )
{
	return portp;
}
...
*/
static inline int amxr_find_dstport(const struct amxr_port_node *portp,
					     /**< Port to look for */
				    const struct amxr_cnxlist_node *cnxlp,
					     /**< Connection list */
...
*     -ve   Failure code
*/
int amxrCoreGetPortInfo(AMXR_HDL hdl,	/*<< (i) Mixer client handle */
...
*     -ve   Failure code
*/
int amxrCoreGetInfo(AMXR_HDL hdl,	/*<< (i) Mixer client handle */
...
*     -ve         Other errors
*/
int amxrCoreSetCnxLoss(AMXR_HDL hdl,	/*<< (i) client handle */
...
*     -ve         Other errors
*/
int amxrCoreGetCnxLoss(AMXR_HDL hdl,	/*<< (i) client handle */
		       AMXR_PORT_ID src_port,	/*<< (i) source port id */
		       AMXR_PORT_ID dst_port,	/*<< (i) destination port id */
		       AMXR_CONNECT_DESC desc,	/*<< (i) Connection descriptor */
...
*/
int amxrCoreConnect(AMXR_HDL hdl,	/*<< (i) client handle */
		    AMXR_PORT_ID src_port,	/*<< (i) source port id */
		    AMXR_PORT_ID dst_port,	/*<< (i) destination port id */
		    AMXR_CONNECT_DESC desc	/*<< (i) Connection descriptor */
    )
{
   ...
   /** Validate connection properties based on port characteristics */
	...
	if (srcportp->info.src_chans != src_chans
	    || src_idx >= src_chans
	    || dstportp->info.dst_chans != dst_chans || dst_idx >= dst_chans) {
	...
	if (src_chans != dst_chans) {
		...
		if ((conn_type == AMXR_CONNECT_STRAIGHT_THROUGH)
		    || (conn_type == AMXR_CONNECT_MULTI_TO_MONO_CONVERT
			&& dst_chans != 1)
		    || (conn_type == AMXR_CONNECT_MONO_TO_MULT_DUPLICATE
			&& src_chans != 1)) {
			AMXR_LOG
			    ("inconsistent multi-channel interleave/deinterleave requested: conn_type=%d src_chans=%d dst_chans=%d",
		...
		if (conn_type != AMXR_CONNECT_STRAIGHT_THROUGH) {
			AMXR_LOG
			    ("should be connected as straight-through: conn_type=%d src_chans=%d dst_chans=%d",
			     conn_type, src_chans, dst_chans);
			return -EINVAL;
		}
	}
	...
	err =
	    amxr_check_resample_rates(srcportp->info.src_hz,
				      dstportp->info.dst_hz, &rtype);
	if (err) {
		AMXR_LOG
		    ("resampling not supported between src_hz=%i and dst_hz=%i",
	...
	if (conn_type == AMXR_CONNECT_STRAIGHT_THROUGH
		...
		AMXR_LOG
		    ("Resampling for multi-channel straight-through unsupported. Split into separate connections with recomposition.");
		return -EINVAL;
	}
...
*     -ve         On general failure
*/
int amxrCoreDisconnect(AMXR_HDL hdl,	/*<< (i) Mixer client handle */
		       AMXR_PORT_ID src_port,	/*<< (i) source port id */
		       AMXR_PORT_ID dst_port	/*<< (i) destination port id */
    )
{
	...
	llist_foreach_item_safe(cnxlp, tmpcnxlp, &gCnxs.disabled_list, lnode) {
		if (cnxlp->srcportp == srcportp) {
			...
			if (amxr_find_dstport(dstportp, cnxlp, &i)) {
				amxr_del_dstport_unsafe(cnxlp, i,
							&gCnxs.disabled_list,
...
*     -ve         On general failure
*/
int amxrCoreGetCnxListBySrc(AMXR_HDL hdl,	/*<< (i) Mixer client handle */
...
*     -ve         On general failure
*/
int amxrCoreGetCnxListByDst(AMXR_HDL hdl,	/*<< (i) Mixer client handle */
    ...
    )
{
	...
	if (cnxlistp->cnxs) {
		...
		portinfop = &cnxlistp->list[0];
		llist_foreach_item(cnxlp, &gCnxs.list, lnode) {
			amxr_info_from_cnxl(&portinfop, cnxlp, dstportp,
					    1 /* active */ );
		}
...
*     -ve      Error code
*/
int amxrCoreCreatePort(const char *name,	/*<< (i) Name string */
...
*/
int amxrCoreRemovePort(AMXR_PORT_ID port	/*<< (i) Port to remove */
    )
{
	struct amxr_port_node *portp;
	struct amxr_cnxlist_node *cnxlp, *tmpcnxlp;
	...
	if (portp->dst_cnxs || portp->src_cnxs) {
		AMXR_LOG
		    ("leftover cnxs on port %s, dst_cnxs=%d src_cnxs%d. Port not deleted.",
...
*     -ve         On general failure
*/
int amxrCoreSetPortDstFreq(AMXR_PORT_ID portid,	/*<< (i) Destination port id */
    ...
    )
{
	...
	AMXR_DEBUG("%s dst_hz=%i dst_bytes=%i", portp->info.name, dst_hz,
		   dst_bytes);
	...
	if (chg_dsthz) {
		...
		llist_foreach_item_safe(cnxlp, tmpcnxlp, &gCnxs.list, lnode) {
			...
			desc =
			    AMXR_CREATE_DESC(cnxlp->cnx_type, cnxlp->src_chans,
					     cnxlp->src_idx,
					     dstl_item.dst_chans,
					     dstl_item.dst_idx);
			...
			if (!err) {
				err =
				    amxr_add_cnx_unsafe(srcportp, portp, desc,
							rtype,
							dstl_item.lossdb);
				if (err) {
					AMXR_LOG
					    ("failed to recreate connection between src=%s and dst=%s src_hz=%d dst_hz=%d",
					     srcportp->info.name,
					     portp->info.name,
					     srcportp->info.src_hz, dst_hz);
					goto fallthrough;
				}
			} else {
				/* Move connection to the disabled list */
				err =
				    amxr_save_disabled_cnx_unsafe(srcportp,
								  &dstl_item,
								  desc);
				if (err) {
					AMXR_LOG
					    ("failed to move connection to disabled list (src=%s and dst=%s src_hz=%d dst_hz=%d)",
					     srcportp->info.name,
					     portp->info.name,
					     srcportp->info.src_hz, dst_hz);
					goto fallthrough;
		...
		llist_foreach_item_safe(cnxlp, tmpcnxlp, &gCnxs.disabled_list,
					lnode) {
			...
			if (amxr_find_dstport(portp, cnxlp, &i)) {
				...
				if (amxr_check_resample_rates
				    (srcportp->info.src_hz, dst_hz,
				     &rtype) == 0) {
					dstlp = cnxlp->dstlp;
					memcpy(&dstl_item, &dstlp[i],
					       sizeof(dstl_item));
					desc =
					    AMXR_CREATE_DESC(cnxlp->cnx_type,
							     cnxlp->src_chans,
							     cnxlp->src_idx,
							     dstl_item.
							     dst_chans,
					...
					err =
					    amxr_add_cnx_unsafe(srcportp, portp,
								desc, rtype,
								dstl_item.
								lossdb);
					if (err) {
						AMXR_LOG
						    ("failed to re-enable connection between src=%s and dst=%s src_hz=%d dst_hz=%d",
						     srcportp->info.name,
						     portp->info.name,
						     srcportp->info.src_hz,
						     dst_hz);
						goto fallthrough;
					}
				}
...
*     -ve         On general failure
*/
int amxrCoreSetPortSrcFreq(AMXR_PORT_ID portid,	/*<< (i) Source port id */
    ...
    )
{
	...
	AMXR_DEBUG("%s src_hz=%i src_bytes=%i", portp->info.name, src_hz,
		   src_bytes);
	...
	if (chg_srchz) {
		...
		llist_foreach_item_safe(cnxlp, tmpcnxlp, &gCnxs.list, lnode) {
			if (cnxlp->srcportp == portp) {
				...
				if (amxr_check_resample_rates
				    (src_hz, dstportp->info.dst_hz,
				     &rtype) == 0) {
					/* Update resampling information for the connection list */
					err =
					    amxr_alloc_resampler(cnxlp, rtype);
					if (err) {
						AMXR_LOG
						    ("failed to update with new resampling info for source port %s src_hz=%d dst_hz=%d",
						     portp->info.name, src_hz,
						     dstportp->info.dst_hz);
						goto fallthrough;	/* quit on error */
					}
				} else {
					/* Delete cnxlist from active list */
					amxr_del_cnxl_unsafe(cnxlp,
							     AMXR_NO_FREE_MEM);

					/* Move the list to the disabled group */
					llist_add_head(&cnxlp->lnode,
						       &gCnxs.disabled_list);
					gCnxs.disabled_total++;
				}
			}
			...
			if (cnxlp->srcportp == portp) {
				...
				if (amxr_check_resample_rates
					...
					for (i = 0; i < cnxlp->cnxs; i++) {
						...
						if (err) {
							AMXR_LOG
							    ("failed to re-add cnx between src=%s and dst=%s src_hz=%d dst_hz=%d",
							     portp->info.name,
							     dstlp[i].portp->
							     info.name, src_hz,
							     dstlp[i].portp->
...
*/
int amxrCoreSetPortDstChannels(AMXR_PORT_ID portid,	/*<< (i) Destination port id */
    ...
    )
{
	...
	AMXR_DEBUG("%s dst_chans=%i dst_bytes=%i", portp->info.name, dst_chans,
		   dst_bytes);
	...
	if (changed && portp->dst_cnxs) {
		...
		llist_foreach_item_safe(cnxlp, tmpcnxlp, &gCnxs.list, lnode) {
			...
			if (amxr_find_dstport(portp, cnxlp, &i)) {
				...
				if (supported) {
					AMXR_DEBUG
					    ("new cnx: cnx_type=%i src_chans=%i dst_chans=%i",
					     cnx_type, src_chans, dst_chans);

					desc =
					    AMXR_CREATE_DESC(cnx_type,
							     src_chans, src_idx,
							     dst_chans,
							     dst_idx);
					...
					if (err) {
						AMXR_LOG
						    ("failed to recreate connection between src=%s and dst=%s src_chans=%d dst_chans=%d",
						     srcportp->info.name,
						     portp->info.name,
						     src_chans, dst_chans);
						mutex_unlock(&gPorts.mutex);
...
*              will be maintained after changing the number of channels.
*/
int amxrCoreSetPortSrcChannels(AMXR_PORT_ID portid,	/*<< (i) Source port id */
    ...
    )
{
	...
	AMXR_DEBUG("%s src_chans=%i src_bytes=%i", portp->info.name, src_chans,
		   src_bytes);
...
*/
static void amxr_resample(const int16_t *srcp,	/*<< (i)  Input data to resample */
    ...
    )
{
	...
	gResampFnc(&resampbufp[filterlen], dstp,	/* output samples */
		   dst_samps,	/* number of 16-bit samples to output */
		   resamptblp->coeffs,
		   filterlen, resamptblp->inter_ratio, resamptblp->decim_ratio);

	/* Update resample buffer history */
	memcpy(resampbufp, srcp + src_samps - filterlen,
	       filterlen * sizeof(int16_t));
}
...
*     This routine is expected to be run in an atomic context.
*/
static inline void amxr_output_samefmt_data(int16_t *dstp,	/*<< (o) dst data ptr */
...
*     This routine is expected to be run in an atomic context.
*/
static inline void amxr_extract_to_mono(int16_t *dstp,	/*<< (o) destination linear buffer ptr */
...
*  @remarks
*/
static inline void amxr_deposit_sub_channel(int16_t *dstp,	/*<< (o) destination interleaved buffer ptr */
...
*     This routine is expected to be run in an atomic context.
*/
static void amxr_straight_through_mix(struct amxr_cnxlist_node *cnxlp,	/*<< (i) cnx list pointer */
    ...
    )
{
	...
	for (i = 0; i < cnxs; i++, dstlp++) {
		...
		if (dstp) {
			amxr_output_samefmt_data(dstp, srcp, dst_samps,
						 dstportp->dst_first,
...
*     This routine is expected to be run in an atomic context.
*/
static void amxr_extract_and_deposit_mix(struct amxr_cnxlist_node *cnxlp,	/*<< (i) cnx list pointer */
					 struct amxr_port_node *targetdstp,	/*<< (i) targeted dest port. If set only service this dst port */
					 int16_t *scratchp	/*<< (o) scratch buffer pointer */
    )
{
	...
	if (src_chans > 1) {
		...
		amxr_extract_to_mono(scratchp, srcp, src_samps, src_chans,
				     src_idx);
		...
		if (dstp) {
			...
			if (dst_chans > 1) {
				...
				if (!(dstportp->dst_first & bmask)) {
					amxr_extract_to_mono(scratchp, dstp,
							     dst_samps,
							     dst_chans,
							     dst_idx);
				}
			...
			if (dst_chans > 1) {
				amxr_deposit_sub_channel(dstportp->dstp, dstp,
							 dst_samps, dst_chans,
							 dst_idx);
			}
		}
...
*     This routine is expected to be run in an atomic context.
*/
static void amxr_convert_to_mono_mix(struct amxr_cnxlist_node *cnxlp,	/*<< (i) cnx list pointer */
				     struct amxr_port_node *targetdstp,	/*<< (i) targeted dest port. If set only service this dst port */
				     int16_t *scratchp	/*<< (o) scratch buffer pointer */
    )
{
...
*     This routine is expected to be run in an atomic context.
*/
static void amxr_duplicate_mono_mix(struct amxr_cnxlist_node *cnxlp,	/*<< (i) cnx list pointer */
				    struct amxr_port_node *targetdstp,	/*<< (i) targeted dest port. If set only service this dst port */
				    int16_t *scratchp	/*<< (o) scratch buffer pointer */
    )
{
	...
	for (i = 0; i < cnxs; i++, dstlp++) {
		...
		if (dstp) {
			...
			for (j = 0, bmask = 1; j < dst_chans; j++, bmask <<= 1) {
				...
				amxr_output_samefmt_data(scratchp, srcp,
							 dst_samps,
							 dstportp->
							 dst_first & bmask,
							 dstlp->lossdb,
				...
				amxr_deposit_sub_channel(dstp, scratchp,
							 dst_samps, dst_chans,
							 j);
			}
			dstportp->dst_first = 0;
...
*     This routine is expected to be run in an atomic context.
*/
static inline void amxr_process_cnxl(struct amxr_cnxlist_node *cnxlp,	/*<< (i) cnx list pointer */
				     struct amxr_port_node *targetdstp,	/*<< (i) targeted dest port. If set only service this dst port */
				     int16_t *scratchp	/*<< (o) scratch buffer pointer */
    )
{
...
*/
void amxrElapsedTime(int elapsed_usec	/*<< (i) Amount of time elapsed since last call in usec */
    )
{
	struct amxr_port_node *portp;
	struct amxr_cnxlist_node *cnxlp;
	...
	llist_foreach_item(portp, &gPorts.list, lnode) {
		if (portp->src_cnxs && AMXR_SYNC_FREQ(portp->info.src_hz)) {
			portp->srcp =
			    portp->cb.getsrc(portp->info.src_bytes,
					     portp->privdata);
		}
		if (portp->dst_cnxs && AMXR_SYNC_FREQ(portp->info.dst_hz)) {
			portp->dstp =
			    portp->cb.getdst(portp->info.dst_bytes,
					     portp->privdata);
	...
	llist_foreach_item(cnxlp, &gCnxs.list, lnode) {
		...
		if (srcportp->srcp == NULL
		    || !AMXR_SYNC_FREQ(srcportp->info.src_hz)) {
	...
	llist_foreach_item(portp, &gPorts.list, lnode) {
		if (portp->cb.dstdone && AMXR_SYNC_FREQ(portp->info.dst_hz)) {
			portp->cb.dstdone(portp->info.dst_bytes,
					  portp->privdata);
		}
		if (portp->cb.srcdone && AMXR_SYNC_FREQ(portp->info.src_hz)) {
			portp->cb.srcdone(portp->info.src_bytes,
					  portp->privdata);
		}
	}
}
...
*     This routine runs in an ATOMIC context!
*/
void amxrCoreServiceUnsyncPort(AMXR_PORT_ID id	/*<< (i) Port ID of port to service */
    )
{
	struct amxr_port_node *portp;
	struct amxr_cnxlist_node *cnxlp;
	...
	llist_foreach_item(cnxlp, &gCnxs.list, lnode) {
		...
		if (srcportp == portp || amxr_find_dstport(portp, cnxlp, NULL)) {
			 ...
			 * entire list 
			 */
			amxr_process_cnxl(cnxlp,
					  (srcportp == portp) ? NULL : portp,
					  gScratchBufp);

			AMXR_TRACE
			    ("src_cnx_count=%i src_cnxs=%i srcportp=0x%lx portp=0x%lx\n",
			     srcportp->src_cnx_count, srcportp->src_cnxs,
			     (unsigned long)srcportp, (unsigned long)portp);

			/* Only send source done it is actually done */
			srcportp->src_cnx_count--;
			if (srcportp->src_cnx_count <= 0) {
				if (srcportp->cb.srcdone) {
					srcportp->cb.srcdone(srcportp->info.
							     src_bytes,
							     srcportp->privdata);
				}
				srcportp->src_cnx_count = srcportp->src_cnxs;
...
*/
static void amxr_dec_dst_cnxcount_unsafe(struct amxr_port_node *dstportp	/*<< (io) Ptr to destination port */
    )
{
	dstportp->dst_cnxs--;
...
*/
static void amxr_free_cnxl_lnode(struct amxr_cnxlist_node *cnxlp	/*<< (i) Ptr to a cnx list node */
    )
{
...
*/
static int amxr_del_dst_from_cnxl_unsafe(struct amxr_cnxlist_node *cnxlp,	/*<< (i) Ptr to a cnx list node */
    ...
    )
{
	...
	for (i = 0; i < cnxlp->cnxs; i++) {
		if (dstportp == dstlp[i].portp) {
			...
			amxr_del_dstport_unsafe(cnxlp, i, &gCnxs.list,
						&gCnxs.total, AMXR_NO_FREE_MEM);
...
*     aquires the connection list mutex.
*/
static int amxr_del_cnxl_unsafe(struct amxr_cnxlist_node *cnxlp,
				     /**< (i) Ptr to connection list to delete */
				int free
				     /**< (i) AMXR_FREE_MEM to free cnxlist memory, 
...
*  @return Nothing
*/
static void amxr_init_cnxdstl_element(struct amxr_cnxdstl_t *dstelemp,
				       /**< (o) Ptr to dst list element */
				      struct amxr_port_node *dstportp,
				       /**< (i) Ptr to dst port node */
				      int dst_chans,
				       /**< (i) number of sub-channels */
				      int dst_idx,
				       /**< (i) sub-channel index to operate on */
				      unsigned int lossdb
				       /**< (i) connection loss value in dB */
    )
{
	uint16_t losslin;
...
*  @remarks
*/
static int amxr_add_cnx(struct amxr_port_node *srcportp,	/*<< (i) Ptr to source port */
			struct amxr_port_node *dstportp,	/*<< (i) Ptr to destination port */
			AMXR_CONNECT_DESC desc,	/*<< (i) Connection descriptor */
    ...
    )
{
	...
	mutex_lock(&gCnxs.mutex);
	err =
	    amxr_add_cnx_unsafe(srcportp, dstportp, desc, rtype,
				0 /* default no loss */ );
	mutex_unlock(&gCnxs.mutex);
...
*/
static int amxr_add_cnx_unsafe(struct amxr_port_node *srcportp,	/*<< (i) Ptr to source port */
			       struct amxr_port_node *dstportp,	/*<< (i) Ptr to destination port */
			       AMXR_CONNECT_DESC desc,	/*<< (i) Connection descriptor */
			       AMXR_RESAMP_TYPE rtype,	/*<< (i) Resampler type */
			       unsigned int lossdb
				       /**< (i) Connection loss in dB */
    )
{
	struct amxr_cnxlist_node *cnxlp;
	int foundcnx;
	...
	if (foundcnx) {
		...
		if (cnxlp->cnxs < cnxlp->max_cnxs) {
			...
			local_irq_save(state);
			amxr_init_cnxdstl_element(&dstlp[cnxlp->cnxs], dstportp,
						  dst_chans, dst_idx, lossdb);
			cnxlp->cnxs++;
			local_irq_restore(state);
		} else {
			...
			newlistp =
			    kmalloc(max_cnxs * sizeof(newlistp[0]),
				    AMXR_MEM_TYPE);
			if (newlistp == NULL) {
				return -ENOMEM;
			}
			...
			memcpy(newlistp, dstlp,
			       sizeof(newlistp[0]) * cnxlp->max_cnxs);

			amxr_init_cnxdstl_element(&newlistp[cnxlp->cnxs],
						  dstportp, dst_chans, dst_idx,
						  lossdb);
		...
		dstlp =
		    kmalloc(AMXR_CNXLIST_SIZE_INCREMENT * sizeof(dstlp[0]),
			    AMXR_MEM_TYPE);
		if (dstlp == NULL) {
			kfree(cnxlp);
			return -ENOMEM;
		...
		memset(dstlp, 0,
		       AMXR_CNXLIST_SIZE_INCREMENT * sizeof(dstlp[0]));
		amxr_init_cnxdstl_element(&dstlp[0], dstportp, dst_chans,
					  dst_idx, lossdb);
...
*/
static int amxr_save_disabled_cnx_unsafe(struct amxr_port_node *srcportp,
				       /**< (i) Ptr to source port */
					 struct amxr_cnxdstl_t *dstl_itemp,
				       /**< (i) Ptr to dst item structure */
					 AMXR_CONNECT_DESC desc
...
*/
static int amxr_alloc_resampler(struct amxr_cnxlist_node *cnxlp,	/*<< (i) Ptr to connection list */
				AMXR_RESAMP_TYPE rtype	/*<< (i) Resampler type */
    )
{
	unsigned long state;
	...
	if (cnxlp->resampbufp == NULL) {
		cnxlp->resampbufp =
		    kmalloc(AMXR_RESAMP_BUFFER_BYTES, AMXR_MEM_TYPE);
		if (cnxlp->resampbufp == NULL) {
			return -ENOMEM;
		}
	}
...
*/
static void amxr_free_resamp(struct amxr_cnxlist_node *cnxlp	/*<< (io) Ptr to connectionn list */
    )
{
...
*/
static void amxr_del_dstport_unsafe(struct amxr_cnxlist_node *cnxlp,
				       /**< (i)  Ptr to a cnx list node */
				    int i,
				       /**< (i)  Dst port index in list */
				    LLIST_NODE *listp,
				       /**< (io) Ptr to list head */
				    int *count,
				       /**< (o)  Ptr to list's total count */
				    int free
				       /**< (i)  AMXR_FREE_MEM to free the memory, 
    ...
    )
{
	...
	/* Squish out destination port */
	memmove(&dstlp[i], &dstlp[i + 1],
		(cnxlp->cnxs - i - 1) * sizeof(dstlp[0]));
	cnxlp->cnxs--;
	if (cnxlp->cnxs == 0) {
...
*/
static void amxr_info_from_cnxl(AMXR_PORT_CNXINFO ** cnxinfopp,
				       /**< (o)  Ptr to cnx info structure */
				struct amxr_cnxlist_node *cnxlp,
				       /**< (i)  Ptr to cnx list */
				struct amxr_port_node *dstportp,
				       /**< (i)  Ptr to dst port used for filtering */
				int active
				       /**< (i)  Flag to indicate active status */
    )
{
	struct amxr_cnxdstl_t *dstlp;
    ...
    )
{
	...
	if (cnxlp->srcportp == srcportp
	    && cnxlp->cnx_type == cnx_type && cnxlp->src_idx == src_idx) {
		struct amxr_cnxdstl_t *dstlp;
		int i;
...
static void __exit amxr_exit(void)
{
	...
	if (gCnxs.disabled_total) {
		AMXR_LOG("orphaned %i disabled connections",
			 gCnxs.disabled_total);
	}
