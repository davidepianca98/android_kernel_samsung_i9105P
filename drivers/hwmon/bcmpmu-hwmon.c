			     ...
			     enum bcmpmu_adc_sig sig)
{
	...
	if (padc->ntc_upper_boundary < temp)
		temp = padc->ntc_upper_boundary;
	return temp;
}

static int adc_map_die_temp(struct bcmpmu_adc *padc, int adc,
			     enum bcmpmu_adc_sig sig)
{
	int temp;
	int offset = padc->die_temp_offset;
	int slope = padc->die_temp_slope;

	temp = (adc * slope + offset)/100;
	return temp;
}
		...
		case PMU_ADC_TM_RTM_SW_TEST:
			...
			if (req->tm == PMU_ADC_TM_RTM_SW_TEST) {
				...
				pr_hwmon(FLOW, "%s: SW_TEST: Pin:%u, "
					 "Gpio:%u, Mux:%u\n", __func__,
					 PN_ADCSYNC, adcsyngpio, adcsyngpiomux);
				/* Setup test pinmuxing */
				StoredPinmux.name  =  PN_ADCSYNC;
				pinmux_get_pin_config(&StoredPinmux);
				TestPinMux.name  =  PN_ADCSYNC;
...
static ssize_t show_dietemp(struct device *dev, struct device_attribute
			*devattr, char *buf)
{
	struct bcmpmu *bcmpmu = dev->platform_data;
	struct bcmpmu_adc_req req;
...
static int __devinit bcmpmu_hwmon_probe(struct platform_device *pdev)
{
	...
	padc->rtm_for_hk = pdata->adc_rtm_for_hk;
	if (pdata->ntc_upper_boundary != 0)
		padc->ntc_upper_boundary = pdata->ntc_upper_boundary;
	else
		padc->ntc_upper_boundary = 1500; /* 150C */
	...
	if (padc->rtm_for_hk == 0) {
		bcmpmu->register_irq(bcmpmu, PMU_IRQ_RTM_DATA_RDY,
			adc_isr, padc);
